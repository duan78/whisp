...
def executer_commande_dev(texte):
    ...
    if "ouvre vs code" in texte or "lance vs code" in texte or "ouvre visual studio code" in texte:
        try:
            ...
            error_handler.log_error(ErrorCategory.DEV_ENVIRONMENT, f"Failed to open VS Code: {e}", ErrorSeverity.MEDIUM)
        ...
        if match:
            ...
            try:
                ...
                error_handler.log_error(ErrorCategory.DEV_ENVIRONMENT, f"Failed to open folder in VS Code: {e}", ErrorSeverity.MEDIUM)
            ...
            try:
                ...
                error_handler.log_error(ErrorCategory.DEV_ENVIRONMENT, f"Failed to open current folder in VS Code: {e}", ErrorSeverity.MEDIUM)
    ...
    # ===== COMMANDES PYCHARM =====
        ...
        try:
            ...
            error_handler.log_error(ErrorCategory.DEV_ENVIRONMENT, f"Failed to open PyCharm: {e}", ErrorSeverity.MEDIUM)
            try:
                # Chemin alternatif pour PyCharm
                subprocess.Popen(["C:\\Program Files\\JetBrains\\PyCharm Community Edition\\bin\\pycharm64.exe"])
                return "PyCharm ouvert"
            except (subprocess.SubprocessError, FileNotFoundError, PermissionError, OSError) as e2:
                error_handler.log_error(ErrorCategory.DEV_ENVIRONMENT, f"Failed to open PyCharm with alternate path: {e2}", ErrorSeverity.MEDIUM)
    ...
    # ===== COMMANDES DOCKER =====
        ...
        try:
            ...
            error_handler.log_error(ErrorCategory.DEV_ENVIRONMENT, f"Failed to run docker ps: {e}", ErrorSeverity.MEDIUM)
        ...
        try:
            ...
            error_handler.log_error(ErrorCategory.DEV_ENVIRONMENT, f"Subprocess error: {e}", ErrorSeverity.MEDIUM)
